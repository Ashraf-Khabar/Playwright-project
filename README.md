...
Our metrics stored now in database, but we need to expose them in grafana, we already have grafana running inside docker container, we open `localhost:3000` and adding our custom dashboards.

#### Connection data source
First we need to connect data source in grafana, adding new data source of type `InfluxDB` , and named it `e2e-metrics-db`, the n setup the parameters :

```
__ Query language : Flux
__ HTTP : http://influxdb:8086
__ Basic auth : disable
__ Orgonization : my-org    //Should match docker compose
__ Token : Influx db
__ Default Bucket : playwright-results
```

# Adding grafana dahaboards
After creating datasource, let's create dashboards that we will help us to monitor our tests.
Latest Run Distribution

## Latest Success Rate
Querie : 
```sql
from(bucket: "playwright-results")
  |> range(start: -30m)
...
from(bucket: "playwright-results")
  ...
  |> last()
  |> group()
  |> map(fn: (r) => ({ r with status_label: if r._value == 1 then "Pass" else "Fail" }))
  |> group(columns: ["status_label"])
  |> count(column: "_value")
  |> group()
```
...
from(bucket: "playwright-results")
  ...
  |> filter(fn: (r) => r["_measurement"] == "test_results")
  |> filter(fn: (r) => r["_field"] == "status")
  |> aggregateWindow(every: 100h, fn: sum, createEmpty: false)
```
Type of graph : `Time series`

<p align="center">
  <img src="./images/grafana-logs.png" width="1000" alt="Diagramme d'architecture">
</p>